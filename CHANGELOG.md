...
### Changed
- **Vendored HyperPress-Core 1.6.1 / HyperBlocks 1.6.1 / HyperFields 1.6.1: WordPress Abilities API support (core 6.9+).** All three libraries register their capabilities as WordPress Abilities: site configuration and the `/wp-html/v1/` endpoint inventory (`hyperpress/*`), block inventory + field definitions + server-side preview rendering (`hyperblocks/*`), and options-page discovery with per-field JSON Schema plus single-field reads and writes (`hyperfields/*`). Everything is registered but hidden by default: sites opt in per package through kill-switch, REST-exposure, and MCP-public filters (see each library's AGENTS.md). Writes go through the exact form-save sanitization pipeline, are `edit_posts`/`manage_options`/per-page-capability gated, and are idempotent. The runtime now boots in every context (REST, WP-CLI, cron) so the Abilities controller can resolve pages and blocks there too; hooks are inert off-context, and the Router rewrite self-heal stays a page-load side effect.

### Added
- **`examples/mcp-server-mu-plugin.php`.** Documented example registering a custom MCP server (official [MCP Adapter](https://github.com/WordPress/mcp-adapter) plugin required) that exposes only the non-destructive Hyper abilities to AI agents. Not loaded by the plugin; copy to `wp-content/mu-plugins/` and adjust. `examples/` is svn-ignored, so it never ships inside the wp.org zip.

### Fixed
- **Windows fixes vendored from all three libraries.** `file:` block templates no longer fail containment validation on Windows (previously a fatal at registration on every request), disabled `_disabled/` JSON blocks can no longer auto-load there, library-mode asset URL detection returns a working URL instead of an empty string, and the endpoint inventory no longer surfaces template names with a leading separator. See each library's 1.6.1 changelog for the details.
...
### Added
- **Adapter test coverage (Brain Monkey).** Added `brain/monkey` (dev) plus a test harness that loads the plugin under Brain Monkey to assert hook wiring: the `hyperpress/admin/show_menu` opt-in, the `system_info` filter the data-tools `admin_menu`, and the activation/deactivation lifecycle. Brain Monkey's `apply_filters` does not execute registered closures, so the brittle pure logic was extracted into testable functions (see Changed) and unit-tested directly. The suite grew from 11 structural tests to 35.

### Changed
- **Behavior-preserving refactor of `bootstrap.php`.** The `system_info` insertion algorithm were extracted into pure functions (`includes/adapter-functions.php`) and called from the closures. Same behavior, now covered by direct unit tests (with `$now` injected for determinism). The nonce-gated `admin_init` dismissal handler is unchanged; it is covered by a wiring assertion only, never a behavior test, because its decision is `wp_verify_nonce()` and a fake check would give false confidence.
- **Bundled library refresh: HyperBlocks 1.5.0 and HyperPress-Core 1.5.2.** HyperBlocks 1.5.0 makes bundled fluent blocks WordPress 7.1 iframed-editor ready (the post editor is always iframed in WP 7.1) by registering them at apiVersion 3, adds the `hyperblocks/blocks/api_version` filter, introduces the `block.json` `"hyperblocks": true` ownership marker so auto-discovery never registers foreign co-located blocks, and fixes JSON-block registration (previously a silent no-op). HyperPress-Core 1.5.2 updates its bundled demo blocks to the new marker/apiVersion and removes the matching facade proxy.
...
- **DEPS:** Updated vendored `estebanforge/hyperfields` to 1.3.6, pulling in two functional releases plus a re-tag (1.3.4 → 1.3.6). The PHP-template field path (options pages and block inspector fields served via `TemplateLoader`) was missing the media picker that only the React path had:
  ...
  - **1.3.4 — flat-layout field styling + double-enqueue fix.** Flat-layout fields (radio/image/file/separator/heading/html) emitted classes with zero CSS rules and rendered as raw HTML; a `:has(> .hyperpress-field-label)` rule now aligns them to the same 220px grid as grid-row fields, with native WP-style rules for radio groups, image/file/media-gallery pickers, separators, headings, and HTML content (mobile collapse under 782px matches grid-row). Also fixed a double-enqueue of `conditional-fields.js` (`Assets::enqueueScripts()` registered it as `hyperfields-conditional-fields` while `TemplateLoader::enqueueAssets()` used `hyperpress-conditional-fields` — same file, two handles, loaded twice with duplicated localization); the `Assets.php` enqueue was removed, leaving `TemplateLoader` the single source.
  - **1.3.5 — empty image-field preview fix.** The PHP image-field templates always emitted the `.hyperpress-image-preview` shell (padding + white bg + border) but only placed an `<img>` inside when a value existed, so an unselected field collapsed to a tiny empty white box. Both rendering paths (`field-image.php` primary and `field-input.php` legacy switch) now render a `<span class="hyperpress-image-placeholder">No image selected</span>` placeholder and add an `is-empty` class when `$value` is empty; `media-fields.js` keeps the placeholder in sync on select/clear; a new `.hyperpress-image-preview.is-empty` CSS rule renders a 150×150 dashed-border box matching populated dimensions (no layout shift on toggle). A `'noImage'` string was added to the admin `hyperpressFields.l10n` array. Both template `echo` sites wrap the class literal in `esc_attr()` as EscapeOutput defense-in-depth.
  - **1.3.6 — re-tag.** Version bump only; no source changes beyond the 1.3.5 fix above (the vendored CHANGELOG tops out at 1.3.5).
- **DEPS:** Bumped vendored `estebanforge/hyperblocks` to 1.2.3 and `estebanforge/hyperpress-core` to 1.3.5 — version-only bumps tracking the hyperfields update (no source changes in either library; their `composer.json` `version` fields are the only modified lines). No HyperPress API or behavior changes.
