 ...
 * Logs are stored in wp-content/uploads/hmapi-logs/.
 */
class Log
{
    ...
    public function handleFatalError()
    {
        ...
        if ($error !== null && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR])) {
            ...
            self::log(self::LOG_LEVEL_CRITICAL, $message, ['source' => 'hmapi-fatal-error']);
        }
    }
     ...
     *                        If 'source' is not provided, 'hmapi-plugin' will be used.
    ...
    public static function log(string $level, string $message, array $context = []): bool
    {
        ...
        if (!self::$logDirSetupDone) {
            if (!self::setupLogDirectory()) {
                ...
                error_log("HMAPI Log Directory Setup Failed. Original log: [{$level}] {$message}");
        ...
        $source = sanitize_file_name($context['source'] ?? 'hmapi-plugin');
        if (empty($source)) {
            $source = 'hmapi-plugin'; // Ensure source is not empty after sanitization
        }
        ...
        if (!error_log($log_entry, 3, $log_file_path)) {
            ...
            error_log("HMAPI Log File Write Failed to {$log_file_path}. Original log: [{$level}] {$message}");
    ...
    private static function setupLogDirectory(): bool
    {
        if (self::$logBaseDir === null) {
            ...
            if (!empty($upload_dir['error'])) {
                error_log('HMAPI Log Error: Could not get WordPress upload directory. ' . $upload_dir['error']);

                return false;
            }
            self::$logBaseDir = $upload_dir['basedir'] . '/hmapi-logs/';
        }

        if (!is_dir(self::$logBaseDir)) {
            if (!wp_mkdir_p(self::$logBaseDir)) {
                error_log('HMAPI Log Error: Could not create log directory: ' . self::$logBaseDir);
        ...
        if (!file_exists($htaccess_file)) {
            ...
            if (@file_put_contents($htaccess_file, $htaccess_content) === false) {
                error_log('HMAPI Log Error: Could not create .htaccess file in ' . self::$logBaseDir);
            }
        }
        ...
        if (!file_exists($index_html_file)) {
            ...
            if (@file_put_contents($index_html_file, $index_content) === false) {
                error_log('HMAPI Log Error: Could not create index.html file in ' . self::$logBaseDir);
            }
        }
